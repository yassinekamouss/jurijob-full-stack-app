{{-- resources/views/emails/recruiter-payment-received.blade.php --}}
<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8" />
  ...
  <title>Paiement reçu — Traitement de votre shortlist – JuriJob</title>
  <style type="text/css">
        ...
        <tr>
          <td class="body-cell" style="padding:40px 48px 32px;color:#1A1F1E;font-family:'Plus Jakarta Sans',Arial,sans-serif;font-size:15px;line-height:1.7;">
            ...
            <p style="margin:0 0 24px;color:#475569;font-size:15px;">
              Nous confirmons la bonne réception de votre virement pour la déblocage de la shortlist <strong style="color:#1A1F1E;">{{ $offre->titre ?? $offre->poste?->label ?? '—' }}</strong>.
            </p>
            ...
            <table role="presentation" cellpadding="0" cellspacing="0" width="100%"
              ...
              @foreach ([
                ...
                ['Montant réglé', $paymentAmount ?? '—'],
                ['Date', $paymentDate ?? '—'],
                ['Statut', 'Paiement en cours de vérification'],
              ] as [$label, $value])
              <tr>
              ...
              <tr>
                <td style="border-left:3px solid #C06041;padding:12px 18px;background-color:#FDF7F5;border-radius:0 6px 6px 0;">
                  <span style="font-family:'Plus Jakarta Sans',Arial,sans-serif;font-size:13px;color:#475569;line-height:1.6;">
                    Dès confirmation administrative de votre règlement (sous <strong style="color:#C06041;">24 heures maximum</strong>), vous recevrez un email avec l'accès complet aux dossiers des candidats.
                  </span>
                </td>
              </tr>
            </table>
            ...
            <p style="margin:0 0 4px;font-family:'Plus Jakarta Sans',Arial,sans-serif;font-size:13px;color:#64748B;">
              Pour transmettre un justificatif : <a href="mailto:{{ config('jurijob.support_email') }}" style="color:#C06041;text-decoration:none;">{{ config('jurijob.support_email') }}</a>
            </p>
